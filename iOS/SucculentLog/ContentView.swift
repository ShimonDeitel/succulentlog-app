...
struct ItemRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text("Species: \(item.primaryField)")
                .font(Theme.captionFont)
                .foregroundColor(Theme.textMuted)
            if !item.secondaryField.isEmpty {
                Text("Pot Size: \(item.secondaryField)")
                    .font(Theme.captionFont)
                    .foregroundColor(Theme.textMuted)
            }
            if !item.subLogs.isEmpty {
                Text("\(item.subLogs.count) Propagation Log entries")
                    .font(Theme.captionFont)
            ...
            Form {
                ...
                Section("Propagation Log") {
                    if purchases.isPurchased {
                        ...
                        TextField("New cutting note", text: $newSubLogNote)
                        ...
                        Button("Add Propagation Log Entry") {
                        ...
                        Button("Unlock Propagation Log with Pro") {
                            showingPaywall = true
                        }
